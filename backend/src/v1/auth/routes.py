...
async def handle_login(
    request: Request, twitter_client: TwitterAuthService = Depends(get_twitter_client)
):
    ...
    try:
        ...
        logger.error(
            f"Connection error while generating auth URL for IP {client_ip}: {str(e)}",
            exc_info=True,
        )
        return {
            "error": "Failed to get OAuth URL: All connection attempts failed"
        }, status.HTTP_500_INTERNAL_SERVER_ERROR
    except httpx.TimeoutException as e:
        logger.error(
            f"Timeout error while generating auth URL for IP {client_ip}: {str(e)}",
            exc_info=True,
        )
        return {
            "error": "Failed to get OAuth URL: Request timed out"
        }, status.HTTP_500_INTERNAL_SERVER_ERROR
    except httpx.HTTPStatusError as e:
        logger.error(
            f"HTTP error while generating auth URL for IP {client_ip}: Status {e.response.status_code}, Response: {e.response.text}",
            exc_info=True,
        )
        return {
            "error": f"Failed to get OAuth URL: HTTP {e.response.status_code}"
        }, status.HTTP_500_INTERNAL_SERVER_ERROR
    except Exception as e:
        logger.error(
            f"Unexpected error while generating auth URL for IP {client_ip}: {str(e)}",
            exc_info=True,
        )
    ...
    try:
        ...
        if error:
            ...
            return RedirectResponse(
                url=f"{config.frontend_url}/error/500?reason={error}", status_code=302
            )

        # Validate required parameters
        if not auth_code or not state:
            return RedirectResponse(
                url=f"{config.frontend_url}/error/500?reason=missing_params",
                status_code=302,
            )

        # Your service handles all the PKCE/session logic
        in_app_token = await twitter_client.fetch_token_store_token(
            authorization_response_url=str(request.url), state=state
        )
        ...
        return RedirectResponse(
            url=f"{config.frontend_url}/callback?{urlencode(params)}", status_code=302
        ...
        return RedirectResponse(
            url=f"{config.frontend_url}/error/500?reason=auth_failed", status_code=302
        )
